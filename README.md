...
Провести кодирование и декодирование текста с помощью *шифра Цезаря*, используя в качестве алфавита стандартную таблицу ASCII. Величина сдвига для каждой позиции в исходном тексте - сумма (по модулю 128) кодов символов слова кодового блокнота, стоящего в блокноте на той же позиции. Если кодовый блокнот имеет количество символов меньше, чем количество символов в исходном тексте, то по исчерпании слов в нем перейти к первому слову и продолжить.
...
$$
\sigma(s_i) = [\gamma(s_i) + \sum_{j = 0}^{|\lambda_i| - 1} \gamma(l_{\lambda_j})] \mod 128\,,
$$
...
где:
 ...
 - $l_{\lambda_j}$ - j-й символ i-го слова в кодовом блокноте;
 - $\gamma(l_{\lambda_j})$ - ASCII код j-го символа i-го слова в кодовом блокноте.
